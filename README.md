...
<img src="docs/screenshots/capture.png" width="360" alt="撮影中の画面。映像と録画時間、コンパクトな許可ボタン、中央の停止ボタン、カメラ切替・マイクのアイコンを表示。">

1. 左側の**カメラ・GPSアイコンのボタン**を押し、両方の利用を許可します。利用可能になるとそれぞれにチェックが付きます。
2. 「録画の画質」を選びます。音声も録音する場合は、右端のマイクアイコンを押してオンにします。斜線付きはオフ、色付きはオンです。
...
右側のカメラ切替アイコンで前後のカメラを切り替えられます。その隣がマイクボタンです。撮影中は画質・カメラ・マイクの変更はできません。

撮影中は、映像・操作ボタンと映像上の録画時間・GPS情報だけを表示します。見出し・ナビゲーション・画質設定・地図・録画一覧・保存欄は隠れ、録画停止後に通常画面へ戻ります。動画の縦横比は変更しません。

携帯を横向きにしても、撮影カードは縦向き時と同程度の幅で中央に表示します。表示枠だけの調整で、録画解像度・動画の縦横比は変更しません。

### どのくらい録画できますか？

**アプリ独自の時間・サイズ上限はありません。通常は停止ボタンを押すまで継続します。**

停止ボタン以外で終了した場合は、再生画面にも終了理由を表示します。フレーム対応表の `recordingEnd` には使用版・終了理由・受信バイト数・動画サイズを記録します。これはアプリが観測できた終了イベントの記録で、OS内部の原因まで特定するものではありません。端末による強制終了など、終了処理が動かなかった場合は残せません。

録画中の動画は、ブラウザ専用の端末内ファイル領域（OPFS）へ順次書き込みます。全動画をメモリに保持して停止後に巨大なBlobを組み立てる方式は使用しません。停止時に書き込み完了とファイルの先頭・末尾の読み出しを確認し、録画一覧へ登録します。端末全体の空き容量をすべて使えるとは限らず、書き込みに失敗すると録画を止めて理由を表示します。

画面が非表示になっただけではアプリから停止しませんが、ブラウザやOSは画面ロック・他アプリへの切替などで撮影を中断することがあります。カメラ切断・録画エラー・ページを閉じる／移動する場合も終了します。**長時間撮影では画面を開いたまま使用してください。** 対応端末ではスリープを抑止し、画面復帰時に再取得します。
...
フルHDは1920 × 1080、4Kは3840 × 2160、HDは1280 × 720です。対応できない設定は端末に合わせて調整されます。4K・60fps・指定ビットレートをすべての端末で保証するものではありません。動き優先では、対応ブラウザでカメラ本来の映像を取得します。iPhone／iPadでは高画質を含む全設定で、対応していればMP4を優先します。プレビューと再生は動画の縦横比を保持します。高画質ほど容量・処理負荷が増えます。
...
> **動画だけを選べば位置情報も読めますか？**
> フォルダ内に当該動画のファイル名とその位置情報ファイル名が存在すれば大丈夫です。
...
| 症状 | 確認すること・対処 |
| --- | --- |
| カメラが表示されない | ブラウザのサイト設定でカメラを許可しているか確認し、許可ボタンを押し直します。|
...
画面最下部に使用中のバージョンを表示します。この修正版は **2026.09.22.3** です。配信ファイルの更新後は版番号を確認してください。更新待ちの場合は案内を表示します。録画中の強制更新や、保存済み録画の削除は行いません。

### 長時間録画後に再生・保存できない／次の録画ができない場合

新しい録画はMP4優先・ファイルへの順次保存に変更しました。Service Workerが有効な場合は、再生・個別保存で巨大なBlob URLを使わず、保存ファイルを必要な範囲だけ読み出します。ZIPも端末内のファイルへ書き出してからダウンロードします。ZIPには元動画とは別の空き容量が必要です。ZIPは録画ごとに1つ保持し、同じ録画の再出力時に置き換え、録画削除時に削除します。

失敗後に表示される「録画状態をリセット」で、カメラ・GPSを再準備できます。未保存の記録を破棄する場合は確認します。保存済みの録画データは削除しません。以前の版で作られたWebMも一覧から開けますが、既に読み出せない・破損した動画の自動修復は行いません。Safariの「WebKitBlobResourceエラー1」だけでは、データが破損しているか、ブラウザが一時ファイルを読めなくなったかは断定できません。
...
- 録画にはOPFSとWorker内の同期ファイル書き込みに対応したブラウザが必要です。非対応・保存権限エラーの場合は録画開始前に通知します。OSによる強制終了時の動画の完全性や、録画一覧への登録は保証できません。
...
`video-file.js` / `video-file-worker.js` がOPFSへの順次書き込みを担当します。Workerは1 MiB以下のバッファで読み書きし、チャンクごとにflush、停止時にcloseします。IndexedDBには新規録画のファイル名・GPS・メタデータを保存し、動画本体を複製しません。旧版のBlob保存形式も読み込み可能です。Service Workerの `__recordings__/` は範囲リクエスト（206）に対応し、動画やZIPを読み出します。撮影設定はmetadataの `capture` に記録します。共有ファイルの生成はWorkerで行い、フレーム対応表は全フレームのオブジェクト配列を一度に生成せず分割してJSON化します。
...
`file-storage.mjs` は一時ブラウザ環境で600 MiBを書き込み、再読み込み、範囲アクセス、ダウンロード、MP4再生を検証します（`STORAGE_TEST_MB` で容量を変更可能）。20分間のiPhone実機録画を代替する試験ではありません。
