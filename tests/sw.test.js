...
function worker(){
  ...
  const self={location:{origin:'https://example.test'},addEventListener:(name,callback)=>events[name]=callback};
  vm.runInNewContext(source,{self,URL,Request:class{constructor(url,options){this.url=url;this.cache=options.cache}},
    caches:{open:async name=>{opened.push(name);return cache},keys:async()=>['geoframe-v8','geoframe-v11','other-app'],delete:async name=>deleted.push(name)},fetch:()=>{throw Error('unexpected network fallback')}});
...
test('new service worker bypasses HTTP cache when precaching a release',async()=>{
  ...
  assert.equal(w.opened[0],'geoframe-v11');assert.ok(w.requests.length>10);
...
test('fetch only reads the current version cache',async()=>{
  ...
  assert.equal((await pending).current,true);assert.deepEqual(w.opened,['geoframe-v11']);
});
